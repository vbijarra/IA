...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (i != j) {
                    peso[i][j] = (double) (imagenC[i] * imagenC[j]) / n; 
                } else {
                    peso[i][j] = 0;
                }
            }
        ...
        entRuidosa[0] = -1;  // Borra esquina superior
        entRuidosa[24] = -1; // Borra esquina inferior
    ...
    static int[] recImagen(int[] entrada, double[][] peso) {    //recorre la entrada 
        ...
        // Límite de iteraciones para evitar bucles infinitos en redes grandes
        int maxIteraciones = 100; 
        int iter = 0;

        while (cambia && iter < maxIteraciones) {      
            ...
            for (int i = 0; i < n; i++) {     //recorre cada pixel de la imagem desde  0 a n-1
                ...
                // Función de activación con estabilidad en cero
                int nuevoVal;
                if (sum > 0) {
                    nuevoVal = 1;
                } else if (sum < 0) {
                    ...
                    nuevoVal = actual[i];   //si la suma es mayor  cero el nuevo valor del pixel será 1 de lo contrario -1
                }
